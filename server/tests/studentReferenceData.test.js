...
describe('OAU student dropdown reference data', () => {
  it('populates the full session range and undergraduate levels with persistent IDs', async () => {
    expect(await seedStudentReferenceData()).toEqual({ sessionsCreated: 65, levelsCreated: 6 });
  ...
  it('can be repeated without changing administrator records, current session, or IDs', async () => {
    ...
    expect(await seedStudentReferenceData()).toEqual({ sessionsCreated: 64, levelsCreated: 5 });
    expect(await seedStudentReferenceData()).toEqual({ sessionsCreated: 0, levelsCreated: 0 });
